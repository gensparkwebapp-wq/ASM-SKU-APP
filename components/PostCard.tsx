    ...
    return (
        <div className="bg-surface-light dark:bg-surface-dark rounded-lg shadow-md border border-gray-200 dark:border-border-dark">
            ...
            <div className="flex items-center gap-3 p-4">
                ...
                <div>
                    <a href={`#profile/${author.id}`} className="font-bold text-gray-900 dark:text-[#e4e6eb] hover:underline">{author.name}</a>
            ...
            <div className="flex justify-around py-1">
                <button 
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg transition-colors disabled:cursor-not-allowed ${
                        userHasLiked ? 'text-primary-blue font-bold' : 'text-gray-600 dark:text-text-secondary hover:bg-gray-100 dark:hover:bg-surface-dark-search'
                    }`}
                >
                    <span className={`material-symbols-outlined text-lg ${userHasLiked ? 'filled' : ''}`}>thumb_up</span>
                        ...
                        {postComments.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(comment => {
                            ...
                            return (
                                <div key={comment.id} className="flex items-start gap-2">
                                    <a href={`#profile/${commentAuthor.id}`}>
                                    ...
                                    <div className="bg-gray-100 dark:bg-surface-dark-search rounded-xl px-3 py-1.5">
                                        <div className="flex items-baseline gap-2">
                                            <a href={`#profile/${commentAuthor.id}`} className="font-bold text-xs hover:underline">{commentAuthor.name}</a>
